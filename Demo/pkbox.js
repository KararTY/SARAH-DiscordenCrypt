...
function pkBox (sodium) {
  ...
  function pkInputMsgUp (e = new KeyboardEvent()) {
    if (e.key === 'Enter' && !e.shiftKey && !e.altKey && !e.ctrlKey && !e.metaKey && e.srcElement.value.length > 0) {
      ...
      pkNewMessage({
        ...
        timestamp: Date.now(),
        message: cryptoUserOne.pkEncrypt(e.srcElement.value, cryptoUserTwo.publicKey)
      })
      e.srcElement.value = '' // Reset value
    }
  }
  function pkInputMsgUpTwo (e = new KeyboardEvent()) {
    if (e.key === 'Enter' && !e.shiftKey && !e.altKey && !e.ctrlKey && !e.metaKey && e.srcElement.value.length > 0) {
      e.preventDefault()
      pkNewMessageTwo({
        author: `${navigator.platform} ${navigator.language} ${navigator.vendor}`,
        timestamp: Date.now(),
        message: cryptoUserTwo.pkEncrypt(e.srcElement.value, cryptoUserOne.publicKey)
      })
      e.srcElement.value = '' // Reset value
    }
  }
  function pkDecryptMessage (e = new MouseEvent()) {
    if (e.srcElement.getAttribute('disabled')) return console.log('Already decrypted.')
    try {
      var message = cryptoUserOne.pkDecrypt(e.srcElement.parentElement.parentElement.querySelector('p.encryptedMessage').innerText, cryptoUserTwo.publicKey)
  ...
  function pkDecryptMessageTwo (e = new MouseEvent()) {
    if (e.srcElement.getAttribute('disabled')) return console.log('Already decrypted.')
    try {
      var message = cryptoUserTwo.pkDecrypt(e.srcElement.parentElement.parentElement.querySelector('p.encryptedMessage').innerText, cryptoUserOne.publicKey)
  ...
  function createChatUI () {
    hyper.bind(document.getElementById('pk'))`
      <div class="section">
        <div class="container">
          <div class="columns">
            <div class="column">
              <div class="box">
                ...
                <br/>
                <label class="label">User One</label>
                <textarea class="textarea" onkeydown=${pkInputMsgDown} onkeyup=${pkInputMsgUp} placeholder="Type in your message..." rows="1"></textarea>
                <label class="label">User Two</label>
                <textarea class="textarea" onkeydown=${pkInputMsgDown} onkeyup=${pkInputMsgUpTwo} placeholder="Type in your message..." rows="1"></textarea>
              </div>
            </div>
          </div>
          <hr>
  ...
  function pkNewMessage (obj = { author: '', timestamp: 0, message: '' }) {
    if (typeof obj !== 'object') throw new Error('pkNewMessage argument is not an object.')
    document.getElementById('pk').querySelector('.messageWrapper').appendChild(hyper.wire()`
      <div class="box content" style="word-break:break-word;">
        <strong>${obj.author}</strong> <small><span title="${window.moment(obj.timestamp).format()}">${window.moment(obj.timestamp).format(notMilitaryTime ? 'h:mm a' : 'HH:mm')}</span> <a onclick=${pkDecryptMessage}>Decrypt with two.</a> </small>
        <p class="encryptedMessage">${obj.message}</p>
      </div>
    `)
  }
  function pkNewMessageTwo (obj = { author: '', timestamp: 0, message: '' }) {
    if (typeof obj !== 'object') throw new Error('pkNewMessage argument is not an object.')
    document.getElementById('pk').querySelector('.messageWrapper').appendChild(hyper.wire()`
      <div class="box content" style="word-break:break-word;">
        <strong>${obj.author} 2</strong> <small><span title="${window.moment(obj.timestamp).format()}">${window.moment(obj.timestamp).format(notMilitaryTime ? 'h:mm a' : 'HH:mm')}</span> <a onclick=${pkDecryptMessageTwo}>Decrypt with one.</a> </small>
        <p class="encryptedMessage">${obj.message}</p>
      </div>
    `)
  }
